...
public class InteractionState : BaseState
{
    ...
    {
        if (stateMachine.layerMask == gunLayer || stateMachine.layerMask == outlineLayer)
            stateMachine.SwitchState(new PickupState(stateMachine));
        else if(stateMachine.objectTag == "Door")
            stateMachine.SwitchState(new DoorOpenState(stateMachine)); 
        else if(stateMachine.objectTag == "Cabinet" || stateMachine.objectTag == "Item")
